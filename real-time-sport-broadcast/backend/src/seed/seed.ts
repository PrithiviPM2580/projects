...
async function readJsonFile(fileUrl: URL): Promise<any> {
...
async function insertCommentary(
  ...
  const responsePayload = (await response.json()) as { data: Commentary };
  return responsePayload.data;
}
...
function cricketBattingTeam(entry: Commentary, match: Match): string | null {
...
function normalizeCricketFeed(
  ...
  const sorted = [...entries].sort((a, b) => {
    ...
    const seqA = Number.isFinite(a.sequence ?? NaN)
      ? (a.sequence ?? Number.MAX_SAFE_INTEGER)
      : Number.MAX_SAFE_INTEGER;
    const seqB = Number.isFinite(b.sequence ?? NaN)
      ? (b.sequence ?? Number.MAX_SAFE_INTEGER)
      : Number.MAX_SAFE_INTEGER;
    if (seqA !== seqB) {
      return seqA - seqB;
    }
    const minA = Number.isFinite(a.minute ?? NaN)
      ? (a.minute ?? Number.MAX_SAFE_INTEGER)
      : Number.MAX_SAFE_INTEGER;
    const minB = Number.isFinite(b.minute ?? NaN)
...
function replaceTrailingTeam(
  ...
  const nextTeam = replacements.get(matchResult[1]);
  if (!nextTeam) {
    return message;
  }
...
function cloneCommentaryEntries(
  ...
  return entries.map((entry) => {
    const next: Commentary = { ...entry, matchId: targetMatch.id };
...
function expandFeedForMatches(
  ...
  const byMatchId = new Map<number, Commentary[]>();
  ...
  const templateBySport = new Map<string, Match>();
  // Note: This would need actual match data to work correctly
...
function buildRandomizedFeed(
  feed: Commentary[],
  matchMap: Map<number, { match: Match }>,
): Commentary[] {
  const buckets = new Map<number | null, Commentary[]>();
  for (const entry of feed) {
...
async function seed(): Promise<void> {
  ...
  const matchMap = new Map<number, { match: Match }>();
